...
if __name__ == '__main__':
    ...
    with open("data/user_train_ml20m.pkl", "wb") as fw:
        pickle.dump(user_train, fw)

    with open("data/user_valid_ml20m.pkl", "wb") as fw:
        pickle.dump(user_valid, fw)

    with open("data/user_test_ml20m.pkl", "wb") as fw:
        pickle.dump(user_test, fw)
    ...
    if args.inference_only:
        ...
        print('Test (NDCG@50: %.4f, HR@50: %.4f)' % (t_test[0], t_test[1]))
    ...
    for epoch in range(epoch_start_idx, args.num_epochs + 1):
        ...
        for step in range(num_batch): # tqdm(range(num_batch), total=num_batch, ncols=70, leave=False, unit='b'):
            ...
            writer.add_scalar("BCEWithLogitsLoss/Train", loss, epoch)
            loss.backward()
            adam_optimizer.step()
        ...
        if epoch % 10 == 0:
            ...
            print("\n", end="")
            print('Epoch: %d, Time: %f(s), Valid (NDCG@50: %.4f, HR@50: %.4f), Test (NDCG@50: %.4f, HR@50: %.4f)'
                    % (epoch, T, t_valid[0], t_valid[1], t_test[0], t_test[1]))
            writer.add_scalar("NDCG@50_validation", t_valid[0], epoch)
            writer.add_scalar("HR@50_validation", t_valid[1], epoch)
            writer.add_scalar("NDCG@50_test", t_valid[0], epoch)
            writer.add_scalar("HR@50_test", t_valid[1], epoch)
            ...
            fname = 'Ml20m_SASRec.epoch={}.lr={}.layer={}.head={}.hidden={}.maxlen={}.pth'
